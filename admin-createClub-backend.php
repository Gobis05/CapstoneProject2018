<?php
  ...
  //Information given from the form
    ...
    //Database info
		...
		$updateQuery = $pdo->prepare('INSERT INTO Club (ClubName, ClubId, ClubEmail, Address, URL, MaxSize, President, PhoneNum, PresidentEmail, AllowedOHRV, Description) VALUES(?,?,?,?,?,?,?,?,?,?,?)');
 		$updateQuery->execute([$name, $id, $email, $address, $url, $maxSize, $president, $phone, $presEmail, $allowed, $descr]);
		echo "Redirecting. Successfully created the club! <script>setTimeout(\"location.href = 'http://turing.plymouth.edu/~mg1021/NHOHVA/Clubs.php';\",3000);</script>";
